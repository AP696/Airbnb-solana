...
export default function Home() {
  ...
  const currentEditListing = useMemo(
    () => listings.find((listing) => listing.id === currentEditListingID),
    [currentEditListingID]
  );
  const displayListings = useMemo(
    () =>
      showReservedListing
        ? listings.filter((listing) => listing.isReserved)
        : listings,
    [showReservedListing, listings]
  );
  ...
  const reserveListing = ({ startDate, endDate }) => {
    ...
    setListings(
      listings.map((listing) => {
        if (listing.id === currentReserveListingID)
          return { ...listing, isReserved: true, reservation: range };
        return listing;
      })
    );
  };
  ...
  const unreserveListing = () => {
    setListings(
      listings.map((listing) => {
        if (listing.id === currentReserveListingID)
          return { ...listing, isReserved: false, reservation: null };
